...
{
	python2 $location/scripts_snp/af-comparison-2.py -mode qtl -f2_mut $f1/F2_filtered.va -f2_wt $f1/control_raw.va -out_test $f1/all_test_variants.va -out_qtl $f1/all_intersect_variants.va -out $f1/F2_control_comparison.va -f_input $f1/$my_gs  2>> $my_log_file 
...
# Run QTL mapping 
{
	...
	python2 $location/scripts_snp/qtl-mapping2.py -in $f1/F2_control_comparison.va -mapqual $problemSample_snpQualityTheshold -out $f1/mapping_info.txt -out2 $f1/candidate_region.txt -width $wd_width -step $wd_step -f_input $f1/$my_gs	-cand_interval $interval_width 2>> $my_log_file
...
# Get INDEL lists
{
	python2 $location/scripts_snp/af-comparison-2.py -mode qtl -f2_mut $f1/F2_raw.va -f2_wt $f1/control_raw.va -out_test $f1/all_raw_test_variants.va -out_qtl $f1/all_raw_candidate_variants.va -out $f1/F2_control_comparison_raw.va -f_input $f1/$my_gs  2>> $my_log_file 
...
# Varanalyzer
{
	touch $f1/varanalyzer_output_snp.txt ;          python2 $location/varanalyzer/varanalyzer.py  -thr $n_threads -itp snp -con $f1/$my_gs -gff $f0/$my_gff -var $f1/snp-to-varanalyzer.txt -rrl $my_rrl -pname $project_name -ann $f0/$my_ann -out $f1/varanalyzer_output_snp.txt 2>> $my_log_file
	touch $f1/varanalyzer_output_total_temp.txt  ;  python2 $location/varanalyzer/varanalyzer.py  -thr $n_threads -itp snp -con $f1/$my_gs -gff $f0/$my_gff -var $f1/snp-to-varanalyzer-total.txt -rrl $my_rrl -pname $project_name -ann $f0/$my_ann -out $f1/varanalyzer_output_total_temp.txt  2>> $my_log_file
	touch $f1/varanalyzer_output_indel.txt       ;  python2 $location/varanalyzer/varanalyzer.py  -thr $n_threads -itp lim -con $f1/$my_gs -gff $f0/$my_gff -var $f1/indel-to-varanalyzer.txt       -rrl $my_rrl -pname $project_name -ann $f0/$my_ann -out $f1/varanalyzer_output_indel.txt  2>> $my_log_file
	touch $f1/varanalyzer_output_indel_total.txt ;  python2 $location/varanalyzer/varanalyzer.py  -thr $n_threads -itp lim -con $f1/$my_gs -gff $f0/$my_gff -var $f1/indel-to-varanalyzer-total.txt -rrl $my_rrl -pname $project_name -ann $f0/$my_ann -out $f1/varanalyzer_output_indel_total.txt  2>> $my_log_file
} || {
